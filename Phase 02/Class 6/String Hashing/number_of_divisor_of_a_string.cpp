...
int32_t main() {
    ...
    string S; cin >> S;
    int n = S.size();
    Hs.prec(); Hs.calculate(S);


    for (int i = 1; i <= n; i++) {
        bool ok = true;
        auto chs = Hs.sub_hash(0, i - 1);
        for (int j = 0; j < n; j += i) {
            if (i + j - 1 < n) {
                ok &= chs == Hs.sub_hash(j, i + j - 1);
            }
            else {
                auto check = Hs.sub_hash(0, n - j - 1);
                ok &= check == Hs.sub_hash(j, n - 1);
            }
        }
        if (ok) {
            cout << i << " ";
