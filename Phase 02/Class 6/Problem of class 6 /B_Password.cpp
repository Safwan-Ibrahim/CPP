...
bool ok(int x) {
    auto l = Hs.sub_hash(0, x - 1);
    for (int i = 1; i + x - 1 < S.size() - 1; i++) {
        if (Hs.sub_hash(i, i + x - 1) == l) {
            return true;
        }
    }
    return false;
...
int32_t main() {
    ...
    vector<int>A;
    for (int i = 0; i < n; i++) {
        if (Hs.sub_hash(0, i - 1) == Hs.sub_hash(n - 1 - i + 1, n - 1)) {
            A.push_back(i);
        }
    }

    int lo = 0, hi = (int) A.size() - 1, ans = 0;
    while(lo <= hi) {
        int mid = (lo + hi) / 2;
        if (ok(A[mid])) {
            ans = A[mid];
