...
struct string_hash {
    ...
    int Pref[2][nn], Pow[2][nn], Ipow[2][nn], Pref2[2][nn];
    ...
    void calculate2(const string& S) {
        int n = S.size();
        int sum0 = 0, sum1 = 0;
    ...
    pair<int, int> sub_hash2(int l, int r) {
        int h0 = (Pref2[0][r] - (l ? Pref2[0][l - 1] : 0) + m0) % m0;
        h0 = 1LL * h0 * Ipow[0][l] % m0;

        int h1 = (Pref2[1][r] - (l ? Pref2[1][l - 1] : 0) + m1) % m1;
        h1 = 1LL * h1 * Ipow[1][l] % m1;
...
bool ok(int x) {
    ...
    for (int i = 0; i + x - 1 < S.size(); i++) {
        St.insert(Hs.sub_hash(i, i + x - 1));
    }
    
    for (int i = 0; i + x - 1 < T.size(); i++) {
        auto p = Hs.sub_hash2(i, i + x - 1);
        if (St.find(p) != St.end()) {
            return true;
        }
    }
...
int32_t main() {
    ...
    Hs.prec();
    Hs.calculate(S), Hs.calculate2(T);
    int lo = 1, hi = n, ans = 0; 
    while(lo <= hi) {
        int mid = (lo + hi) / 2;
        if (ok(mid)) {
    ...
    for (int i = 0; i + ans - 1 < S.size(); i++) {
        St.insert(Hs.sub_hash(i, i + ans - 1));
    }
    
    for (int i = 0; i + ans - 1 < T.size(); i++) {
        auto p = Hs.sub_hash2(i, i + ans - 1);
        if (St.find(p) != St.end()) {
            cout << T.substr(i, ans) << endl; return 0;
        }
    }
